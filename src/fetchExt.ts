...
export const outerFetcher = baseConfig => {
    const parser = createParser(baseConfig);

    return initParser(parser);
}

const initParser = parser => uriBase => responseActions => planStr => uriParams => {
    ...
    return innerFetcher(config)
}

const innerFetcher = config => async (payload?) => {
    debugger
